...
const stripePromise = loadStripe("pk_test_51MIv3QGkHBBLcQz0cX4vyas4So6hxiTiuXGWCD83Ufz4zDqBGtMCzJGSq6PNj21gPOik8bzuBMWh9nMRWbxpdHBg00UAZ1H7nQ");
      ...
      <Container style={{ minHeight: '95vh' }} className='cart-container'>
         <Row>
            <Col md={6}>
               ...
               {cart.length === 0 ? (
                  <Alert variant="info" className='text-center'>Shopping cart is empty. Add products to your cart</Alert>
               ) : (
                        ...
                        <tbody>
                           ...
                           {cart.map((item) => (
                              <tr>
                                 ...
                                 <td>${(Math.round(item.price * user.cart[item._id] * 100) / 100).toFixed(2)}</td>
                              </tr>
                           ))}
                        </tbody>
                     </Table>
                     <div>
                        <h3 className="h4 pt-4">Total: ${(Math.round(user.cart.total * 100) / 100).toFixed(2)}</h3>
                     </div>
                  </>
               </Col>
            )}
